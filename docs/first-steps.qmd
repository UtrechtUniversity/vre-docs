...
### Attaching storage to a paused workspace
You can also attach a storage volume to a pause workspace by expanding the workspace tile, selecting 'Storage' from the options, and then attaching the relevant storage volume. This is useful if you forgot to attach a storage volume when creating a workspace, or if you want to attach a different storage volume.
