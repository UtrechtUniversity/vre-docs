...
def server(input, output, session):
    ...
    def device_controls():
        if input.device() == "CPU":
            ...
            return ui.TagList(
                ui.input_slider("gpus", "Number of GPU drives", 1, 4, 1, step=1),
                ui.input_numeric("time", "Estimated time", 12, min=1, max=50),
                ui.input_radio_buttons(
                    "time_unit", "Time Unit", ["hours", "days"]
                    ),
    ...
    def plot():
        ...
        if input.device() == "CPU":
            ax.axhline(y=cost[input.time()], xmin=0, xmax=input.time() / 50, color='r', linestyle='dashed')
        else:
            ax.axhline(y=cost[input.time()], xmin=0, xmax=input.time() / 20, color='r', linestyle='dashed')
