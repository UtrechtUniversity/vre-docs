...
ASReview uses an AI model that runs in parallel to the ASReview webapplication. **It is therefore recommended to choose a workspace with at least two CPU cores. Four CPUs is probably enough for most cases.** The more users that will be using the workspace to do screening in parallel, the more hardware will be required.
