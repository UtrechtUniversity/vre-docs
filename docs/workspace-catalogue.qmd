...
This page provides an overview of workspace types (called "Catalog Items" on Research Cloud) supported by Utrecht University and some commonly used Catalog Items developed by SURF.
...
All UU users have access to UU items. To use UU items listed on this page, you can simply find them in the ResearchCloud portal under the 'Catalog' tab and [request access](manuals/catalog-item-access.qmd). Most catalog items maintained by SURF will already be automatically visible for every Collaborative Organisation.

Look at the specific manual for your desired workspace to find out more about how to access it. 
...
We recommend using [VRE Lab](workspaces/programming/vre-lab.qmd) for most use cases to develop and run your code as it comes with a wide range of software that you can easily preinstall. 
...
def display_category(name, headers=table_headers):
    ...
    workspaces = sorted(workspaces, key=lambda d: d.get('title', '') != 'VRE Lab')
    if len(workspaces) > 0:
