...
If you are not working with an environment manager, we recommend using [`uv`](https://docs.astral.sh/uv/). Start by listing the packages you import in your code (e.g., `pandas`, `numpy`, `matplotlib`) and create a new `requirements.txt` file with one package per line.
