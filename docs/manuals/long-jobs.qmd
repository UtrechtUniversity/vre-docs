...
- When you access your workspace via your web browser (VRE Lab, RStudioUU, Ubuntu Desktop, etc.)
...
- In Jupyterhub interface (VRE Lab Workspace in Research cloud): 
...
- In Rstudio (RstudioUU workspace in Research cloud):
...
1. The script will prompt you for your ResearchCloud API token. Enter it. You will not see the token as you paste it, this is normal. After you have pasted the token, press Enter.
...
If for instance, your script contains errors, the workspace will still be paused as the script cannot continue. You can check the log file to find out what went wrong, and then resume the workspace to work on your files again. 
...
You can try the following example to test the use of `run_and_pause`.

:::{.callout-note collapse=true #test-example}
## Example Script to Test Running Long Jobs
...
def main():
    ...
    for step in range(1, total_steps + 1):
        log(f"Step {step}/{total_steps} started")
        result = fake_work(step)
        elapsed = time.time() - start_time
        avg_time = elapsed / step
        remaining = avg_time * (total_steps - step)
        print(progress_bar(step, total_steps), flush=True)
        log(
    ...
    log(f"Total runtime: {total_time:.1f} seconds")

if __name__ == "__main__":
    main()
```
Then open a terminal and run the following command and follow the instructions:
``` bash
run_and_pause "python test.py"

run_and_pause -s python main.py # to run in silent mode
```
The script will run for about 5 minutes, and will print progress updates. After the script is finished, the workspace will be paused automatically. You can check the log file to see the progress updates and final output of the script. To see the progress in real time, run the `tail -f /home/youruser/run_and_pause.abcd123.log` command as shown in the terminal.
:::
...
For this usecase we combine `nohup` with an ampersand "&" to run a background process. The script will continue even if the terminal window is closed. The process will only stop:
...
To practice using `nohup`, you can use the [example script here](#test-example), and run it with the following command:
``` bash
