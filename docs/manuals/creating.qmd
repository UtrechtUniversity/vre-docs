...
You can inspect the available configurations for each Cloud Operator by expanding the 'Available sizes' context (`^`):

<img alt="Available Cloud Sizes Example" width="50%" src="../imgs/cloud_sizes.png" />

After selecting a Cloud Operator and Flavour, you can get more info on costs by expanding the `^` context menu. See [here](../responsible-use.qmd#cost-calculator) for a cost calculator (only for the SURF HPC cloud!).
